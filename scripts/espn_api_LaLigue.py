...
# ---------- CONFIG ----------
LEAGUE_CODE = "fra.1"   # Ligue 1 (Francia)
...
def extract_stats_from_summary_json(summary_json):
    """
    summary_json is the response from:
    https://site.api.espn.com/apis/site/v2/sports/soccer/{league}/summary?event={match_id}
    We will try to extract possessionPct, totalShots, shotsOnTarget, wonCorners, yellowCards, redCards
    """
    out = {
    ...
    try:
        ...
        teams = box.get("teams") or summary_json.get("boxscore", {}).get("teams") or []
        for team in teams:
            is_home = False
            if team.get("homeAway"):
                is_home = team.get("homeAway") == "home"
            stats_list = team.get("statistics") or team.get("stats") or []
            for s in stats_list:
                ...
                if val is None:
                    continue
                if "possession" in name or "possessionpct" in name:
                    if is_home:
                        out["posesion_local"] = normalize_num(val)
                    else:
                        out["posesion_visitante"] = normalize_num(val)
                elif "total" in name and ("shot" in name or "shots" in name):
                    if is_home:
                        out["tiros_totales_local"] = normalize_num(val)
                    else:
                        out["tiros_totales_visitante"] = normalize_num(val)
                elif "on target" in name or "shotsontarget" in name or "shots on target" in name or "shots on goal" in name:
                    if is_home:
                        out["tiros_a_puerta_local"] = normalize_num(val)
                    else:
                        out["tiros_a_puerta_visitante"] = normalize_num(val)
                elif "corner" in name:
                    if is_home:
                        out["corners_local"] = normalize_num(val)
                    else:
                        out["corners_visitante"] = normalize_num(val)
                elif "yellow" in name:
                    if is_home:
                        out["tarjetas_amarillas_local"] = normalize_num(val)
                    else:
                        out["tarjetas_amarillas_visitante"] = normalize_num(val)
                elif "red" in name:
                    if is_home:
                        out["tarjetas_rojas_local"] = normalize_num(val)
                    else:
                        out["tarjetas_rojas_visitante"] = normalize_num(val)
        return out
    ...
    try:
        ...
        if comps:
            ...
            for team in comp.get("competitors", []):
                is_home = team.get("homeAway", "") == "home"
                tname = team.get("team", {}).get("displayName") or team.get("team", {}).get("name") or team.get("displayName")
                score = team.get("score")
                rec_name = "equipo_local" if is_home else "equipo_visitante"
                rec_score = "goles_local" if is_home else "goles_visitante"
                rec[rec_name] = tname
                try:
                    rec[rec_score] = int(score) if score is not None and score != "" else None
                except:
                    rec[rec_score] = score
        return rec
    except Exception as e:
...
def scrape_range(start_date, end_date, league_code):
    ...
    for d in tqdm(list(daterange(start_date, end_date)), desc="Fechas"):
        ...
        if not resp:
            print(f" Error request scoreboard {datestr}")
            continue
        try:
            data = resp.json()
        except Exception as e:
        ...
        for event in events:
            ...
            try:
                if rec["fecha"]:
                    if isinstance(rec["fecha"], str) and len(rec["fecha"]) == 8 and rec["fecha"].isdigit():
...
def main():
    ...
    if total == 0:
        print(" No se extrajeron partidos. Posibles causas: fechas fuera de cobertura ESPN para esa liga, bloqueo por IP, o errores temporales del API.")
    if recs:
        ...
        print(f" Archivo JSONL creado vacío: {OUT_JSONL}")
